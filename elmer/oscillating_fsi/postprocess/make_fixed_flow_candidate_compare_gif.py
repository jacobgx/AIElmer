"""Render fixed-flow candidate comparison GIFs."""
...
CASES = (
    CaseSpec("fixed_flow_nostab_outfine_5s", "attempted reduction, 555 N", 0.01),
    CaseSpec("fixed_flow_refined_linear_bubbles_5s", "linear refined + bubbles, 124 N", 0.05),
    CaseSpec("fixed_flow_bubbles_5s", "bubbles, 52 N", 0.05),
)
...
def color_limits(case_root: Path, times: np.ndarray) -> tuple[float, float]:
    values = []
    for case in CASES:
        for time in times[:: max(1, len(times) // 10)]:
...
def render_panel(ax: plt.Axes, file: Path, label: str, vmin: float, vmax: float) -> object:
    ...
    ax.triplot(fluid, color="black", linewidth=0.035, alpha=0.07)
    ...
    if len(solid_triangles):
        ...
        ax.triplot(solid, color="black", linewidth=0.28, alpha=0.85, zorder=5)
...
def render_compare(case_root: Path, output: Path, start: float, end: float, frames: int, fps: float) -> Path:
    ...
    images = []
    for time in times:
        fig, axes = plt.subplots(1, 3, figsize=(15.5, 3.5), dpi=130, sharex=True, sharey=True)
        contour = None
...
def main() -> None:
    ...
    output = args.output or args.case_root / "visualizations" / "fixed_flow_candidate_refined_linear_compare_t4to5.gif"
